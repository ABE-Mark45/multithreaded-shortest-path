...
class Client {
 ...
 public:
  Client(const std::string& name, const std::string& inputPath,
  ...
  auto sendNextBatch() {
    ...
    client_.sendQueryBatch(result, name_, batch);
    ...
    logger_.println("Batch was calculated in " +
...
int main(int argc, char** argv) {
  ...
  if (argc != 4) {
    throw std::runtime_error("./client <name> <input-path> <log-path>");
  }
  Client client(argv[1], argv[2], argv[3]);

  do {
    // auto numSeconds = 1 + rand() % 10;
